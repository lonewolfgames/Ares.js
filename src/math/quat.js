...
define([
	 ...
	 * @return this
	 */
        Quat.prototype.lookRotation = function(){
            var dup = new Vec3( 0, 0, 1 ),
		dforward = new Vec3( 0, 1, 0 ),
		axis = new Vec3;
	    
	    return function( forward, up ){
		up = up || dup;
		( forward = forward || dforward ).normalize();
		
		var dot = forward.dot( dforward ),
		    angle = 0;
		
		if( abs( dot + 1 ) < EPSILON ) return this.set( up.x, up.y, up.z, PI );
		if( abs( dot - 1 ) < EPSILON ) return this.set( 0, 0, 0, 1 );
		
		angle = acos( dot );
		axis.vcross( forward, up ).normalize();
	 ...
	 * @method rotationTo
	 * @memberof Quat
	 * @brief creates a rotation represent the shortest rotation from one vector to another
	 * @param Vec3 a
	 * @param Vec3 b
	 * @return this
	 */
        Quat.prototype.rotationTo = function(){
            var vec = new Vec3,
		xUnit = new Vec3( 1, 0, 0 ),
		yUnit = new Vec3( 0, 1, 0 );
		...
		if( dot < -ONE ){
		    ...
		    if( vec.lengthSq < EPSILON ) vec.vcross( yUnit, a );
		    
		    vec.normalize();
		    this.fromAxisAngle( vec, PI );
	 ...
	 * @return this
	 */
        Quat.prototype.fromVec3s = function( u, v ){
	    var ux = u.x, uy = u.y, uz = u.z,
		vx = v.x, vy = v.x, vz = v.z,
	    ...
	    this.y = y;
	    this.z = z;
	    this.w = sqrt( ( ux * ux + uy * uy + uz * uz ) * ( vx * vx + vy * vy + vz * vz ) ) + ( ux * vx + uy * vy + uz * vz );
	 ...
	 * @method fromEuler
	 * @memberof Quat
	 * @brief sets values from Euler
	 * @param Euler e
	 * @return this
	 */
        Quat.prototype.fromEuler = function( e ){
	    var c1 = cos( e.x * 0.5 ),
		c2 = cos( e.y * 0.5 ),
		c3 = cos( e.z * 0.5 ),
		s1 = sin( e.x * 0.5 ),
	 ...
	 * @method vequals
	 * @memberof Quat
	 * @brief checks if a's values equal b's values, can be called as a static function Quat.vequals( a, b )
	 * @param Quat a
	 * @param Quat b
	 * @return Boolean
	 */
        Quat.vequals = Quat.prototype.vequals = function( a, b ){
