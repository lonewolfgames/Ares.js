...
define([
	 ...
	 */
	    ...
	    var u = new Vec3, v = new Vec3, uv = new Vec3,
		faceNormal = new Vec3;
	...
	Mesh.prototype.fromJSON = function( json ){
		...
		colors = this.colors,
		uvs = this.uvs,
		bones = this.bones, bone, bonei, bonej,
		...
		bones.push(
			...
			bindPose: new Mat4().fromArray( item.bindPose ),
			inheritRotation: item.inheritRotation,
			inheritScale: item.inheritScale
		    })
		);
	    }
	...
	Mesh.Sphere = function( radius, segments, rings ){
		...
		uvs = mesh.uvs, colors = mesh.colors,
		faces = mesh.faces;
